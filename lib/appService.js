...
/**
 * Force-refresh the session tokens after a governed change was applied.
 *
 * Committing a change request re-signs the user's identity, so their current
 * access/ID tokens (and the kcToken cookie the middleware verifies) are stale
 * until refreshed. Each attempt is bounded by a timeout and never throws: a
 * refresh that hangs or rejects must not wedge the UI, and the caller needs to
 * know whether the session survived so it can recover instead of leaving the
 * user on a broken token.
 *
 * @param {object} auth - the auth context (needs forceUpdateToken)
 * @param {object} [options] - attempts / delayMs / timeoutMs overrides
 * @returns {Promise<boolean>} - true if at least one refresh succeeded
 */
async function refreshTokensAfterCommit(auth, options = {}) {
    const { attempts = 3, delayMs = 1000, timeoutMs = 15000 } = options;
    let refreshed = false;

    for (let attempt = 1; attempt <= attempts; attempt++) {
        // Give the backend a moment to propagate the committed change first.
        await new Promise((resolve) => setTimeout(resolve, delayMs));
        try {
            await Promise.race([
                auth.forceUpdateToken(),
            ...
            console.warn(`[refreshTokensAfterCommit] attempt ${attempt}/${attempts} failed:`, error?.message || error);
        }
    }
