...
async function getUser(baseURL, realm, token, userId){
    try {
        ...
        if (!response.ok){
            throw(`Fetch Error: Unable to get user with id ${userId}.`)
        }
...
async function getUserAttributes(baseURL, realm, userId, token){
    try {
        const response = await fetch(`${baseURL}/admin/realms/${realm}/users/${userId}/unmanagedAttributes`, { 
            method: 'GET',
            headers: {
                "Content-Type": "application/json",
                "authorization": `Bearer ${token}`,
        ...
        if (!response.ok) {
           throw ("Unable to get user info");
        }
    
        return {vuid: response["vuid"], tideUserKey: response["tideUserKey"]};
    }
    catch (error) {
        console.log(error);
    }
