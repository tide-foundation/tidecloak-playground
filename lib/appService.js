...
/**
 * Get raw change set request data for approval (matching ideed-SWARM's GetRawChangeSetRequest)
 * @param {string} baseURL - url body provided in the context
 * @param {string} realm - the realm name provided in the context
...
async function reviewChangeRequest(baseURL, realm, usersChangeReq, token){
    try {
        // Use batch endpoint like ideed-SWARM does
        const response = await fetch(`${baseURL}/admin/realms/${realm}/tide-admin/change-set/sign/batch`, {
            method: 'POST',
            headers: {
                "Content-Type": "application/json",
                "authorization": `Bearer ${token}`,
        ...
        // Convert base64 to bytes (matching ideed-SWARM's base64ToBytes)
